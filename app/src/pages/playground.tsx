import React, {
  useCallback, useContext, useEffect, useRef, useErrorBoundary
} from "react"
import {
  Editor,
  EditorState,
...
import {styleMap, getDecoratedStyle} from "../lib/editorStyles"
import { APIContext, EditorContext, ModelsContext, ModelsStateContext, ParametersContext, HistoryContext} from "../app"
import ParameterSidePanel from "../components/parametersSidePanel"
...
const PromptCompletionEditor = ({showDialog}) => {
  ...
  const handleStreamingSubmit = async (
    ...
    const _cancel_callback = apiContext.completion_request({
  ...
  useEffect(() => {
    const completion_callback = ({event, data, meta}) => {
      switch (event) {
        case "cancel":
          setGenerating(false)
        break;
    ...
    apiContext.subscribe_completion(completion_callback)

    return () => {
      apiContext.unsubscribe_completion(completion_callback);
    };
  }, []);
